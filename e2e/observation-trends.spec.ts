 ...
 */
async function createActivityAndGenerateShow(page: import('@playwright/test').Page, activityName: string) {
 ...
 */
async function generateShowAndRecordObservations(page: import('@playwright/test').Page, activityId: string) {
...
test.describe('Observation Trends (WP2.2)', () => {
	test('Observation section appears on activity detail after recording observations', async ({ page }) => {
	...
	test('Drill-through shows session observations', async ({ page }) => {
		...
		if (await obsLabels.count() > 0) {
			...
			await expect(page.getByText(/Positive|Neutral|Needs work/).first()).toBeVisible({ timeout: 3000 });
		}
	});
});
