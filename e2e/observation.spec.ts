 ...
 */
async function createActivityAndShowToClass(page: import('@playwright/test').Page, activityName: string) {
...
test.describe('Observation UI (WP2.1)', () => {
	...
	test('Note input persists after sentiment tap until dismissed', async ({ page }) => {
		...
		// Tap "+" on first group
		await page.getByRole('button', { name: /Positive observation/i }).first().click();
		...
		// Dismiss with ✕ button
		await page.getByRole('button', { name: /Dismiss note input/i }).first().click();
