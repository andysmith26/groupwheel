<script lang="ts">
  ...
  function looksLikeRoster(headers: string[]): boolean {
    ...
    const hasName = lowerHeaders.some(
      (h) => h.includes('name') || h.includes('student') || h === 'first' || h === 'last'
    );

    // Check for email column
    const hasEmail = lowerHeaders.some((h) => h.includes('email') || h.includes('e-mail'));
  ...
  function looksLikeResponses(headers: string[]): boolean {
    ...
    const hasChoiceColumns = lowerHeaders.some(
      (h) => h.includes('choice') || h.includes('[') // matrix questions have [option] format
    );
  ...
  function saveRecentSheet(url: string, title: string) {
    ...
    const updated: RecentSheet[] = [{ url, title, lastUsed: Date.now() }, ...filtered].slice(
      0,
      MAX_RECENT_SHEETS
    );
   ...
   */
  function buildStudentsFromMatchResult(): {
