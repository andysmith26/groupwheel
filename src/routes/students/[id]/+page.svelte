<script lang="ts">
  ...
  onMount(async () => {
    ...
    if (!isOk(result)) {
      loadError =
        result.error.type === 'IDENTITY_NOT_FOUND'
          ? 'Student not found'
          ...
          <div>
            ...
            {#if profile}
              <p class="text-sm text-gray-500">
                ...
                {profile.summary.activityCount}
                {profile.summary.activityCount === 1 ? 'activity' : 'activities'}
                {profile.summary.totalGroupings > 0
                  ? ` · ${profile.summary.totalGroupings} groupings`
                  : ''}
              </p>
            {/if}
          </div>
    ...
    {:else if loading}
      <div class="flex items-center justify-center py-12">
        <div class="flex flex-col items-center gap-2">
          <div
            class="h-8 w-8 animate-spin rounded-full border-2 border-gray-300 border-t-teal"
          ></div>
