<script lang="ts">
	...
	import ExpandablePoolCard from '$lib/components/ExpandablePoolCard.svelte';
	...
	let poolStudents: Map<string, Student[]> = new Map();
	let programPreferences: Preference[] = [];
	...
	async function loadProgram(id: string) {
		...
		if (isOk(result)) {
			...
			// Load students for each pool
			for (const pool of pools) {
				const students: Student[] = [];
				for (const memberId of pool.memberIds) {
					const student = await env.studentRepo.getById(memberId);
					if (student) {
						students.push(student);
					}
				}
				poolStudents.set(pool.id, students);
			}
			// Trigger reactivity
			poolStudents = poolStudents;

			// Load preferences for this program
			programPreferences = await env.preferenceRepo.listByProgramId(program.id);
		} else {
	...
	$: unassignedStudentIds = scenarioResult
		? scenarioResult.participantSnapshot.filter(
				(id) => !scenarioResult?.groups.some((group) => group.memberIds.includes(id))
			)
		: [];
	...
	{:else if program}
		<section class="space-y-3">
			<div class="flex items-center justify-between">
				<h2 class="text-lg font-semibold">Pools & Student Preferences</h2>
				{#if programPreferences.length > 0}
					<span class="text-sm text-gray-500">
						{programPreferences.length} student preferences loaded
					</span>
				{/if}
			</div>
			...
			{:else}
				{#each pools as pool (pool.id)}
					<ExpandablePoolCard
						{pool}
						students={poolStudents.get(pool.id) ?? []}
						preferences={programPreferences.filter((p) => pool.memberIds.includes(p.studentId))}
						expanded={expandedPoolId === pool.id}
						on:toggle={() => (expandedPoolId = expandedPoolId === pool.id ? null : pool.id)}
					/>
				{/each}
			{/if}
		</section>
		...
		<section class="space-y-4 rounded-lg border bg-white p-4 shadow-sm">
			...
			{#if scenarioResult}
				<div class="space-y-2 rounded-lg border bg-gray-50 p-3 text-sm text-gray-700">
					<div class="flex flex-wrap items-center justify-between gap-3">
						<div class="space-y-1">
							<p class="text-xs tracking-wide text-gray-500 uppercase">Scenario</p>
