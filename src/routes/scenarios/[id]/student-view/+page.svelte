<script lang="ts">
	...
	onMount(async () => {
		...
		const studentsById = new SvelteMap<string, Student>();
...
{:else if viewData}
	<div class="min-h-screen bg-white p-8">
		<header class="mb-8 text-center">
			<h1 class="text-4xl font-bold text-gray-900">Student Groups</h1>
			<p class="mt-2 text-xl text-gray-600">
				{viewData.groups.length} groups · {viewData.groups.reduce(
		...
		<div class="grid gap-6 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
			{#each viewData.groups as group (group.id)}
