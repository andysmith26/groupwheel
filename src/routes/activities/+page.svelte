<script lang="ts">
  ...
  let qsStudentCountValid = $derived(
    !isNaN(qsStudentCountNum) && qsStudentCountNum >= 2 && qsStudentCountNum <= 200
  );
  let qsGroupSizeValid = $derived(
    !isNaN(qsGroupSizeNum) && qsGroupSizeNum >= 2 && qsGroupSizeNum <= 20
  );
  let qsCanCreate = $derived(
    qsStudentCountValid && qsGroupSizeValid && qsGroupSizeNum <= qsStudentCountNum
  );
...
<div class="mx-auto max-w-4xl space-y-6 p-4">
  <header class="flex items-center justify-between gap-4">
    ...
    <div class="flex items-center gap-3">
      <Button href="/analytics" variant="secondary">Analytics</Button>
      <Button href="/activities/import" variant="secondary">Import</Button>
      <Button href={newActivityHref} variant="primary">+ New Activity</Button>
    </div>
  </header>
  ...
  <div class="rounded-lg border-2 border-amber-200 bg-amber-50 p-6 text-amber-900">
    ...
    <p class="mt-2 text-sm text-amber-900/90">
      Activities are saved only in this browser. Other people cannot see them. To share or
      permanently save an activity, use the Export option on the editing page.
    </p>
  </div>
        ...
        <div class="mt-4 flex flex-wrap items-end gap-4">
          <div>
            ...
            <input
              ...
              max="200"
              inputmode="numeric"
              class="mt-1 block w-28 rounded-md border border-gray-300 px-3 py-2 text-center shadow-sm focus:border-teal focus:ring-1 focus:ring-teal focus:outline-none"
              ...
              <div class="min-w-0 flex-1">
                <div class="flex items-center gap-2">
                  {#if isDemoActivity(activity)}
                    <span
                      class="flex-shrink-0 rounded-full bg-purple-100 px-2 py-0.5 text-xs font-medium text-purple-700"
                    >
                      Demo
                    </span>
                  {/if}
                  <h3 class="truncate font-medium text-gray-900 group-hover:text-teal">
                    {isDemoActivity(activity)
                      ? activity.program.name.replace(/^Demo: /, '')
                ...
                >
                  {#if isDemoActivity(activity)}
                    <button
                      ...
                      onclick={() => {
                        openMenuId = null;
                        handleDeleteDemo(activity.program.id);
                      }}
                    >
                      Delete Demo
                    </button>
