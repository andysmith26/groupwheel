<script lang="ts">
  ...
  let totalPossiblePairs = $derived(studentCount > 1 ? (studentCount * (studentCount - 1)) / 2 : 0);
  ...
  {:else if program}
    ...
    <div class="mb-6">
      ...
      <div class="mt-3 flex items-start justify-between">
        <div class="min-w-0 flex-1">
          {#if isEditingName}
            <div class="flex items-center gap-2">
              <input
                type="text"
                class="block w-full rounded-md border border-gray-300 px-3 py-1.5 text-xl font-semibold shadow-sm focus:border-teal focus:ring-1 focus:ring-teal focus:outline-none"
          ...
          {:else}
            ...
            >
              ...
              <h1 class="text-2xl font-semibold text-gray-900">
                {isDemoActivity ? program.name.replace(/^Demo: /, '') : program.name}
              </h1>
              {#if !isDemoActivity}
                <svg
        ...
        <span class="mt-1 ml-4 text-sm text-gray-500">
          {studentCount}
    ...
    {#if hasPlaceholderStudents && !placeholderBannerDismissed}
      <div
        class="mb-4 flex items-center justify-between rounded-lg border border-blue-200 bg-blue-50 px-4 py-3 text-sm text-blue-800"
      >
        <div>
          ...
          <button
            ...
            onclick={() => {
              if (expandedSection !== 'students') handleSectionToggle('students')(true);
            }}
          >
            Setup &rarr;
          </button>
    ...
    {#if isDemoActivity}
      <div
        class="mb-4 flex items-center justify-between rounded-lg border border-purple-200 bg-purple-50 px-4 py-3 text-sm text-purple-800"
      >
        ...
        >
          <div>
            ...
            <span class="ml-2 text-sm text-gray-500">
              {coveragePercent}% of pairings covered · {pairingSessionCount}
              {pairingSessionCount === 1 ? 'session' : 'sessions'}
            </span>
          </div>
          <svg
            class="h-5 w-5 text-gray-400 transition-transform {expandedSection === 'rotation'
              ? 'rotate-90'
              : ''}"
            fill="none"
            stroke="currentColor"
