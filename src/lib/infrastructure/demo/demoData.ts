...
import type { Student, Pool, Program, Preference, Scenario, Session, GroupTemplate, Staff, Placement } from '$lib/domain';
...
const FIRST_NAMES = [
	...
	'Aiden', 'Olivia', 'Liam', 'Emma', 'Noah', 'Ava', 'Elijah', 'Sophia',
	'James', 'Isabella', 'William', 'Mia', 'Benjamin', 'Charlotte', 'Lucas',
	'Amelia', 'Henry', 'Harper', 'Alexander', 'Evelyn', 'Mason', 'Abigail',
	'Michael', 'Emily', 'Ethan', 'Elizabeth', 'Daniel', 'Sofia', 'Jacob', 'Ella',
	// Additional diverse names
	'Jayden', 'Camila', 'Logan', 'Luna', 'Jackson', 'Chloe', 'Sebastian', 'Penelope',
	'Mateo', 'Layla', 'Jack', 'Riley', 'Owen', 'Zoey', 'Theodore', 'Nora',
	'Amir', 'Lily', 'Santiago', 'Eleanor', 'Isaiah', 'Hannah', 'Ryan', 'Addison',
	'Nathan', 'Aubrey', 'Leo', 'Stella', 'Adrian', 'Natalie', 'Caleb', 'Leah',
	'Kai', 'Savannah', 'Ezra', 'Brooklyn', 'Connor', 'Aria', 'Aaron', 'Skylar',
	// More diverse names
	'Hiroshi', 'Priya', 'Mohammed', 'Fatima', 'Wei', 'Mei', 'Dmitri', 'Anastasia',
	'Carlos', 'Maria', 'Andre', 'Jasmine', 'Yusuf', 'Zara', 'Kofi', 'Amara',
	'Raj', 'Ananya', 'Kenji', 'Sakura', 'Omar', 'Aaliyah', 'Ivan', 'Tatiana',
	'Luis', 'Valentina', 'Kwame', 'Nia', 'Hassan', 'Laila', 'Jun', 'Yuki'
];
...
const LAST_NAMES = [
	...
	'Mitchell', 'Carter', 'Roberts', 'Chen', 'Kim', 'Patel', 'Singh', 'Wong',
	'Park', 'Tanaka', 'Sato', 'Yamamoto', 'Okonkwo', 'Mensah', 'Ahmed', 'Khan',
	'Sharma', 'Kumar', 'Ivanov', 'Petrov', 'Santos', 'Silva', 'Costa', 'Ferreira'
];
...
function generatePreferences(
	...
	for (const student of students) {
		...
		if (random() < 0.30 && students.length > 5) {
		...
		if (random() < 0.40 && groupIds.length > 0) {
		...
		if (random() < 0.10 && groupIds.length > 1) {
...
function generateGroups(configs: GroupConfig[]): { id: string; name: string; capacity: number; memberIds: string[] }[] {
...
export function generateDemoData(seed: number = 42): DemoDataSet {
	...
	const scienceLabPrefs = generatePreferences(scienceLabProgram.id, class1Students, scienceLabGroups.map(g => g.id), random);
	...
	const readingPrefs = generatePreferences(readingProgram.id, class1Students, readingGroups.map(g => g.id), random);
	...
	const mathPrefs = generatePreferences(mathProgram.id, class1Students, mathGroups.map(g => g.id), random);
	...
	const ecoPrefs = generatePreferences(ecoProgram.id, class2Students, ecoGroups.map(g => g.id), random);
	...
	const class2LabPrefs = generatePreferences(class2LabProgram.id, class2Students, class2LabGroups.map(g => g.id), random);
	...
	const debatePrefs = generatePreferences(debateProgram.id, class3Students, debateGroups.map(g => g.id), random);
	...
	const geoPrefs = generatePreferences(geoProgram.id, class3Students, geoGroups.map(g => g.id), random);
	...
	const clubPrefs = generatePreferences(clubProgram.id, clubStudents, clubGroups.map(g => g.id), random);
	...
	const sessionScenarioPrefs: Array<{ session: Session; scenario: Scenario; preferences: Preference[] }> = [
