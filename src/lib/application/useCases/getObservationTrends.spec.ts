...
describe('getObservationTrends', () => {
  ...
  it('returns empty summaries when no observations exist', async () => {
    await sessionRepo.save(
      makeSession({ id: 's1', programId: 'p1', createdAt: new Date('2026-01-01') })
    );
    await sessionRepo.save(
      makeSession({ id: 's2', programId: 'p1', createdAt: new Date('2026-01-02') })
    );
    await sessionRepo.save(
      makeSession({ id: 's3', programId: 'p1', createdAt: new Date('2026-01-03') })
    );
  ...
  it('sessionLimit caps results to most recent N sessions', async () => {
    for (let i = 1; i <= 5; i++) {
      await sessionRepo.save(
        makeSession({ id: `s${i}`, programId: 'p1', createdAt: new Date(`2026-01-0${i}`) })
      );
      await observationRepo.save(
        makeObservation({
          id: `o${i}`,
