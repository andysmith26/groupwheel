...
export type GetStudentProfileError = { type: 'IDENTITY_NOT_FOUND'; message: string };
...
export async function getStudentProfile(
  ...
  const programIds = [...new Set(Array.from(sessionMap.values()).map((s) => s.programId))];
  ...
  const pairingStats = await calculatePairingStats(deps, studentIds, allPlacements);

  // Calculate summary
  const summary = calculateSummary(identity, placementHistory, programIds.length);
...
function calculateSummary(
  ...
  const firstChoiceCount = placementHistory.filter((p) => p.placement.preferenceRank === 1).length;
  const placementsWithPrefs = placementHistory.filter(
    (p) => p.placement.preferenceRank !== null
  ).length;
  const firstChoicePercentage =
    placementsWithPrefs > 0 ? Math.round((firstChoiceCount / placementsWithPrefs) * 100) : 0;
