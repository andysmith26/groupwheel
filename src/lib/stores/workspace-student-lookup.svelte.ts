...
export function createWorkspaceStudentLookup(deps: WorkspaceStudentLookupDeps) {
  ...
  let tooltipStudent = $derived.by(() => {
    ...
    return tc.state.studentId ? (deps.getStudentsById()[tc.state.studentId] ?? null) : null;
  });

  let tooltipPreferences = $derived.by(() => {
    const tc = deps.getTooltipController();
    return tc.state.studentId ? (deps.getPreferenceMap()[tc.state.studentId] ?? null) : null;
  });
  ...
  let sidebarStudent = $derived.by(() => {
    ...
    return sc.state.studentId ? (deps.getStudentsById()[sc.state.studentId] ?? null) : null;
  });

  let sidebarPreferences = $derived.by(() => {
    const sc = deps.getSidebarController();
    return sc.state.studentId ? (deps.getPreferenceMap()[sc.state.studentId] ?? null) : null;
  });
