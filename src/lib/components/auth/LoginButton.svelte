...
{:else}
  <div class="relative" bind:this={menuRef}>
    <button
      ...
      class="flex items-center gap-2 rounded-full border border-transparent px-1.5 py-1 text-sm font-medium text-gray-700 hover:border-gray-200 hover:bg-gray-50 focus:ring-2 focus:ring-teal focus:ring-offset-2 focus:outline-none"
    >
      {#if user?.avatarUrl}
        <img
          src={user.avatarUrl}
      ...
      {:else if user}
        <div
          class="flex h-7 w-7 items-center justify-center rounded-full bg-blue-100 text-sm font-medium text-blue-600"
        >
        ...
        {#if user}
          <div class="px-4 py-3">
            ...
            <div class="mt-3 rounded-md border border-gray-200 bg-gray-50 px-3 py-2">
              <p class="text-xs tracking-wide text-gray-400 uppercase">Active sheet</p>
