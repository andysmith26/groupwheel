...
<div
  ...
  class={`group flex h-full w-full flex-col gap-4 rounded-xl border bg-white p-4 text-left shadow-sm transition focus-visible:ring-2 focus-visible:ring-slate-400 focus-visible:outline-none ${
    disabled
...
>
  <div class="flex items-center justify-between">
    <div>
      <div class="text-xs font-semibold tracking-wide text-slate-400 uppercase">{label}</div>
  ...
  <div class="grid grid-cols-2 gap-3 text-sm">
    <div class="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2">
      <div class="text-xs font-semibold text-slate-500 uppercase">Top Choice</div>
      <div class="text-base font-semibold text-slate-900">{formattedTopChoice}</div>
    </div>
    <div class="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2">
      <div class="text-xs font-semibold text-slate-500 uppercase">Avg Rank</div>
      <div class="text-base font-semibold text-slate-900">{formattedAverageRank}</div>
    </div>
  </div>

  <div class="rounded-lg border border-slate-200 bg-white p-3">
    <div class="text-xs font-semibold tracking-wide text-slate-500 uppercase">Group Preview</div>
    <div class="mt-2">
          ...
          <div class="rounded-md border border-slate-100 bg-slate-50/60 p-2">
            ...
            <div class="mt-1 text-slate-600">
              {#if group.memberIds.length === 0}
                <span class="text-slate-400 italic">No students assigned</span>
              {:else}
