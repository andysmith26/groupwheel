...
{#if visible}
  <div
    class="fixed right-4 bottom-4 z-50 w-80 rounded-lg border border-gray-200 bg-white shadow-xl"
    role="alert"
    aria-live="polite"
  >
    <!-- Header -->
    ...
    <!-- Content -->
    <div class="p-4">
      <p class="mb-3 text-sm text-gray-600">How did it go?</p>

      <!-- Sentiment buttons -->
      <div class="mb-3 flex items-center gap-2">
        <button
          type="button"
          class="flex-1 rounded-lg border px-3 py-2 text-lg transition-colors {selectedSentiment ===
          'POSITIVE'
      ...
      {:else}
        ...
        <textarea
          ...
          class="w-full resize-none rounded-lg border border-gray-200 px-3 py-2 text-sm focus:border-transparent focus:ring-2 focus:ring-teal focus:outline-none"
          rows="3"
        ></textarea>
        ...
        <div class="mt-3 flex items-center justify-end gap-2">
          ...
          <button
            type="button"
            class="rounded-lg bg-teal px-4 py-1.5 text-sm font-medium text-white hover:bg-teal-dark disabled:opacity-50"
            onclick={handleSave}
