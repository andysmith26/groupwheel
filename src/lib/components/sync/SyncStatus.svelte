<script lang="ts">
	...
	onMount(() => {
		if (syncService) {
			unsubscribe = syncService.onStatusChange((newStatus: SyncStatus) => {
				status = newStatus;
			});
		}
	});
	...
	const tooltipText = $derived.by(() => {
		if (!status.enabled) return 'Sync disabled (not signed in)';
		if (status.syncing) return 'Syncing...';
...
{#if status.enabled}
	<div
		class="sync-status flex items-center gap-1.5 text-xs"
		title={tooltipText}
	>
		{#if statusIcon === 'syncing'}
			...
			<span class="hidden text-gray-600 sm:inline">Syncing</span>
		...
		{:else if statusIcon === 'offline'}
			...
			<span class="hidden text-gray-500 sm:inline">Offline</span>
		{:else if statusIcon === 'pending'}
			...
			<span class="hidden text-gray-600 sm:inline">{status.pendingChanges} pending</span>
		{:else}
			...
			<span class="hidden text-gray-600 sm:inline">Synced</span>
		{/if}
	</div>
{/if}
