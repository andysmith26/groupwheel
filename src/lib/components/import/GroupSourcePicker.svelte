...
<div class="space-y-4">
  ...
  <div class="grid gap-3 {hasTemplates ? 'grid-cols-3' : 'grid-cols-2'}">
    {#if hasPreferenceData}
      <button
        ...
        {disabled}
        class="rounded-lg border-2 p-4 text-left transition-colors {selectedSource === 'preferences'
    ...
    {#if hasConnection}
      ...
      >
        <div class="flex items-center gap-3">
          <div
            class="flex h-10 w-10 items-center justify-center rounded-lg {selectedSource === 'sheet'
              ? 'bg-teal/10 text-teal'
              : 'bg-gray-100 text-gray-500'}"
          >
            <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      {:else if extractedGroups.length > 0}
        <div class="space-y-3">
          <p class="text-sm font-medium text-gray-700">
            Found {extractedGroups.length} group{extractedGroups.length === 1 ? '' : 's'} from student
            choices
          </p>
          <div class="flex flex-wrap gap-2">
            {#each extractedGroups as group}
              <span
                class="inline-flex items-center gap-1 rounded-full border border-gray-200 bg-white px-3 py-1 text-sm"
              >
                {group.name}
  ...
  {#if selectedSource === 'sheet' && connection}
    <div class="space-y-4 rounded-lg border border-gray-200 bg-gray-50 p-4">
      ...
      {#if tabData}
        <div class="space-y-3">
          <p class="text-sm text-gray-600">Map the column containing group names:</p>
          <SheetPreview
            data={tabData}
          ...
          {:else}
            <p class="text-sm text-gray-500">Select a column to use as group names</p>
          {/if}
        </div>
      {/if}
    </div>
