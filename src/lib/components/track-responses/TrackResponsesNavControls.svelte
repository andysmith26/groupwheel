...
<div class="flex flex-1 items-center gap-3">
  <div class="flex items-center gap-2">
    ...
    <select
      ...
      onchange={(event) =>
        (trackResponsesSession.stateFilter = (event.target as HTMLSelectElement)
          .value as StudentStateFilter)}
    >
      <option value="all">All ({trackResponsesSession.counts.total})</option>
      <option value="not_submitted"
        >Not submitted ({trackResponsesSession.counts.unresolved})</option
      >
    ...
    <input
      ...
      class="h-9 w-full max-w-xs min-w-[180px] rounded-md border border-gray-300 px-3 text-sm focus:border-teal focus:ring-1 focus:ring-teal focus:outline-none"
