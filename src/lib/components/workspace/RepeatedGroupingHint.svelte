...
{:else}
	...
	<label class="flex items-center gap-2 rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 cursor-pointer hover:bg-gray-100 transition-colors">
		<input
			type="checkbox"
			class="h-4 w-4 rounded border-gray-300 text-teal focus:ring-teal"
			checked={checked}
		...
		<span class="text-sm text-gray-700">
			Different groups than last session
		</span>
	</label>
{/if}
