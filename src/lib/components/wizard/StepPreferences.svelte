<script lang="ts">
  ...
  import { parseAllMatrixPreferences, extractChoiceRank } from '$lib/utils/matrixPreferenceParser';
  ...
  {:else}
    ...
    {#if hasSheetConnection}
      <div class="flex gap-2">
        <button
          type="button"
          class="flex-1 rounded-lg border-2 px-4 py-3 text-sm font-medium transition-colors {importSource ===
          'sheet'
        ...
        {#if formatDetection && tabData}
          <div class="space-y-4 rounded-lg border border-blue-200 bg-blue-50 p-4">
            <div class="flex items-start gap-3">
              ...
              <div class="flex-1">
                ...
                {#if formatDetection.previewGroups.length > 0}
                  <div class="mt-2">
                    <p class="mb-1 text-xs text-blue-600">Groups found:</p>
                    <div class="flex flex-wrap gap-1">
                      {#each formatDetection.previewGroups as group}
                        <span
                          class="inline-flex items-center rounded bg-blue-100 px-2 py-0.5 text-xs text-blue-800"
                        >
                          {group}
                        </span>
                      {/each}
            ...
            <div class="border-t border-blue-200 pt-3">
              <label for="student-id-column" class="mb-2 block text-sm font-medium text-blue-900">
              ...
              <select
                ...
                onchange={(e) =>
                  handleStudentIdColumnChange(parseInt((e.target as HTMLSelectElement).value))}
              >
              ...
              {#if studentIdColumnIndex >= 0}
                <p class="mt-1 text-xs text-blue-600">
                  Preview: {tabData.rows
                    .slice(0, 2)
                    .map((r) => r.cells[studentIdColumnIndex])
                    .join(', ')}...
                </p>
              {/if}
            </div>
            ...
            {#if previewPreferences.length > 0}
              <div class="border-t border-blue-200 pt-3">
                <p class="mb-2 text-sm font-medium text-blue-900">Preview of parsed preferences:</p>
                <div class="divide-y divide-blue-100 rounded-lg border border-blue-100 bg-white">
                  {#each previewPreferences as item}
                    <div class="px-3 py-2">
                      <p class="text-sm font-medium text-gray-900">{item.studentName}</p>
