  ...
  function handleTabSelect(tab: SheetTab, data: RawSheetData) {
    ...
    if (detection.isMatrixFormat && detection.columns.length > 0) {
      ...
      if (extracted.length > 0) {
        ...
        sheetError =
          'Could not detect group names in this tab. Try a tab with headers like "[Group Name]" or select groups manually.';
      }
    }
  }
...
<div class="space-y-6">
        ...
        <div class="space-y-3">
          ...
          <div class="grid gap-3 sm:grid-cols-3">
            ...
            <button
              ...
              class="flex items-center gap-3 rounded-xl border-2 p-3 text-left transition-colors {groupSource ===
              'manual'
            ...
            >
              <div
                class="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-lg {groupSource ===
                'manual'
                  ? 'bg-teal/20'
                  : 'bg-gray-100'}"
              >
                <svg
                  class="h-4 w-4 {groupSource === 'manual' ? 'text-teal' : 'text-gray-600'}"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                >
              ...
              <div class="min-w-0 flex-1">
                <div class="flex items-center gap-1.5">
                  <span
                    class="text-sm font-medium {groupSource === 'manual'
                      ? 'text-teal-dark'
                      : 'text-gray-900'}"
                  >
                    Enter manually
            ...
            {#if hasTemplates}
              ...
              >
                ...
                >
                  <svg
                    class="h-4 w-4 {groupSource === 'template' ? 'text-teal' : 'text-gray-600'}"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                  >
                ...
                <div class="min-w-0 flex-1">
                  <div class="flex items-center gap-1.5">
                    <span
                      class="text-sm font-medium {groupSource === 'template'
                        ? 'text-teal-dark'
                        : 'text-gray-900'}"
                    >
                      Use template
            ...
            {#if hasSheetConnection}
              ...
              >
                ...
                >
                  <svg
                    class="h-4 w-4 {groupSource === 'sheet' ? 'text-teal' : 'text-gray-600'}"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                  >
                ...
                <div class="min-w-0 flex-1">
                  <div class="flex items-center gap-1.5">
                    <span
                      class="text-sm font-medium {groupSource === 'sheet'
                        ? 'text-teal-dark'
                        : 'text-gray-900'}"
                    >
                      Import from sheet
        ...
        <div class="space-y-4 rounded-lg border border-gray-200 bg-gray-50 p-4">
          ...
          {#if detectedGroups.length > 0}
            <div class="rounded-lg border border-green-200 bg-green-50 p-4">
              <p class="mb-2 text-sm font-medium text-green-900">
  ...
  {:else}
    <div class="rounded-lg border border-gray-200 bg-gray-50 p-4">
      <p class="text-center text-sm text-gray-500">Select an option above to continue.</p>
    </div>
  {/if}
</div>
