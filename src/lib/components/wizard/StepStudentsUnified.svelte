<script lang="ts">
  ...
  // Simple name list detection
  let simpleListDetected = $state(false);
  ...
  function parseSheetData(data: RawSheetData) {
    ...
    try {
      ...
      if (nameIdx < 0 && firstNameIdx < 0 && idIdx < 0) {
        parseError =
          'Please map at least one column to "Full Name", "First Name", or "ID / Email".';
        onStudentsParsed([]);
        return;
      }
  ...
  function parseRoster() {
    ...
    try {
      // Check for simple name list first (no delimiters, one name per line)
      const nameListResult = detectSimpleNameList(pastedText);
      if (nameListResult.isSimpleNameList) {
        const parsed: ParsedStudent[] = nameListResult.names.map((name) => {
  ...
  function switchToColumnMapping() {
    simpleListDetected = false;
    parseRosterAsCSV();
  }
...
<div class="space-y-4">
  <div>
    ...
    <p class="mt-1 text-sm text-gray-600">Choose how you'd like to add your students.</p>
  </div>
  ...
  <div class="space-y-3">
    ...
    <div
      class="overflow-hidden rounded-xl border-2 transition-colors {expandedSection === 'paste'
    ...
    >
      ...
      >
        <div
          class="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-lg {expandedSection ===
          'paste'
            ? 'bg-teal/20'
            : 'bg-gray-100'}"
        >
          <svg
            class="h-5 w-5 {expandedSection === 'paste' ? 'text-teal' : 'text-gray-600'}"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
          >
        ...
        <div class="min-w-0 flex-1">
          <div class="flex items-center gap-2">
            <span
              class="font-medium {expandedSection === 'paste' ? 'text-teal-dark' : 'text-gray-900'}"
            >
              Paste from spreadsheet
            </span>
          ...
          <p
            class="mt-0.5 text-sm {expandedSection === 'paste'
              ? 'text-teal-dark/80'
              : 'text-gray-500'}"
          >
      ...
      {#if expandedSection === 'paste'}
        <div class="space-y-4 border-t border-teal/20 p-4">
          <div class="space-y-2">
...
Bob Jones	bob@school.edu	9"
    ...
    {#if hasExistingRosters}
      <div
        class="overflow-hidden rounded-xl border-2 transition-colors {expandedSection === 'roster'
          ? 'border-teal bg-teal-light/30'
          : 'border-gray-200 hover:border-gray-300'}"
      >
        <button
        ...
        >
          ...
          >
            <svg
              class="h-5 w-5 {expandedSection === 'roster' ? 'text-teal' : 'text-gray-600'}"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
          ...
          <div class="min-w-0 flex-1">
            <div class="flex items-center gap-2">
              <span
                class="font-medium {expandedSection === 'roster'
                  ? 'text-teal-dark'
                  : 'text-gray-900'}"
              >
                Use existing roster
              ...
              <span
                class="text-sm {expandedSection === 'roster'
                  ? 'text-teal-dark/70'
                  : 'text-gray-500'}"
              >
            ...
            <p
              class="mt-0.5 text-sm {expandedSection === 'roster'
                ? 'text-teal-dark/80'
                : 'text-gray-500'}"
            >
          ...
          <div class="space-y-3 border-t border-teal/20 p-4">
            ...
            <div class="max-h-72 space-y-3 overflow-y-auto pr-1">
              {#if hasRosterQuery}
                ...
                {:else}
                  <div class="space-y-2">
                    <p class="text-xs font-medium tracking-wide text-gray-500 uppercase">Results</p>
              ...
              {:else}
                {#if recentRosters.length > 0}
                  <div class="space-y-2">
                    <p class="text-xs font-medium tracking-wide text-gray-500 uppercase">Recent</p>
    ...
    {#if userLoggedIn}
      <div
        class="overflow-hidden rounded-xl border-2 transition-colors {expandedSection === 'sheet'
          ? 'border-teal bg-teal-light/30'
          : 'border-gray-200 hover:border-gray-300'}"
      >
        <button
        ...
        >
          ...
          >
            <svg
              class="h-5 w-5 {expandedSection === 'sheet' ? 'text-teal' : 'text-gray-600'}"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
          ...
          <div class="min-w-0 flex-1">
            <div class="flex items-center gap-2">
              <span
                class="font-medium {expandedSection === 'sheet'
                  ? 'text-teal-dark'
                  : 'text-gray-900'}"
              >
                Import from Google Sheets
            ...
            <p
              class="mt-0.5 text-sm {expandedSection === 'sheet'
                ? 'text-teal-dark/80'
                : 'text-gray-500'}"
            >
        ...
        {#if expandedSection === 'sheet'}
          <div class="space-y-4 border-t border-teal/20 p-4">
            {#if hasSheetConnection && sheetConnection}
              ...
              {#if isAutoDetecting}
                <div
                  class="flex items-center gap-2 rounded-lg border border-blue-200 bg-blue-50 p-3"
                >
                    ...
                    <div class="mt-2 flex flex-wrap items-center gap-2">
                      ...
                      {#if detectedGroupNames.length > 5}
                        <span class="text-xs text-blue-600"
                          >+{detectedGroupNames.length - 5} more</span
                        >
                      {/if}
                    </div>
              ...
              {#if showMappingUI && tabData}
                <div class="space-y-3">
                            ...
                            <th
                              ...
                              mapping !== 'ignore'
                                ...
                                : mapping === 'ignore'
                                  ...
                                  : 'text-gray-600'}"
                            >
                              <span class="block max-w-[120px] truncate" title={header}
                                >{header}</span
                              >
                            </th>
                          {/each}
                  ...
                  <button
                    ...
                    class="rounded-md bg-teal px-4 py-2 text-sm font-medium text-white hover:bg-teal-dark disabled:cursor-not-allowed disabled:opacity-50"
                  >
                    Import {tabData.rows.length} Students
                  </button>
                </div>
              {/if}
            {:else if onSheetConnect}
              <SheetConnector
              ...
              />
            {:else}
              <p class="text-sm text-gray-600">Sheet connection is not available.</p>
            {/if}
          </div>
        {/if}
      </div>
  ...
  {#if students.length > 0}
    {#if simpleListDetected}
      <!-- Simplified preview for simple name lists -->
      <div class="rounded-lg border border-green-200 bg-green-50 p-4">
        ...
        <ul class="ml-7 space-y-1 text-sm text-gray-700">
          {#each students.slice(0, 4) as student}
            <li>{student.displayName}</li>
          {/each}
          {#if students.length > 4}
            <li class="text-gray-500">...and {students.length - 4} more</li>
          {/if}
        </ul>

        <p class="mt-3 ml-7 text-xs text-gray-500">
          Not what you expected?
          <button
            type="button"
            class="text-teal-600 underline hover:text-teal-700"
            onclick={switchToColumnMapping}
          >
    ...
    {:else}
      <!-- Full table preview for CSV/TSV data -->
