<script lang="ts">
  ...
  let groupNames = $derived.by(() => {
    ...
    return groupConfig.groups.map((g) => (g.name ?? '').trim()).filter((name) => name.length > 0);
  });
  ...
  let showNameError = $derived.by(() => (isEditingName ? !draftName.trim() : !activityName.trim()));
...
<div class="space-y-6">
  <div>
    ...
    <p class="mt-1 text-sm text-gray-600">Review your setup and create your groups.</p>
  </div>
  ...
  <div class="rounded-lg border border-gray-200 bg-gray-50 p-4">
    <h3 class="mb-3 text-sm font-medium text-gray-700">Summary</h3>

    <div class="space-y-3">
      <!-- Students -->
      <div class="flex items-start gap-3">
        <span
          class="flex h-6 w-6 flex-shrink-0 items-center justify-center rounded-full bg-teal-100"
        >
          <svg
            class="h-3.5 w-3.5 text-teal-600"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
      ...
      <div class="flex items-start gap-3">
        <span
          class="flex h-6 w-6 flex-shrink-0 items-center justify-center rounded-full bg-amber-100"
        >
          <svg
        ...
        <div>
          <div class="font-medium text-gray-900">
            {stats.groupCount}
            {stats.allGroupsNamed ? 'named groups' : 'groups'}
          </div>
          <div class="text-sm text-gray-500">
    ...
    <div class="mt-4 border-t border-gray-200 pt-3">
      <p class="text-xs text-gray-500">
        Groups will be assigned randomly. After creating, you can import student preferences from
        Setup to refine placement.
      </p>
    </div>
  </div>
</div>
