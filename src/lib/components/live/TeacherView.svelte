...
{:else}
	<div class="relative">
		...
		<button
			type="button"
			class="fixed bottom-6 right-6 z-20 flex h-14 w-14 items-center justify-center rounded-full bg-teal text-white shadow-lg transition-transform hover:scale-105 active:scale-95"
		...
		{#if showQuickNote}
			...
			<div class="fixed bottom-0 left-0 right-0 z-40 rounded-t-2xl bg-white p-6 shadow-2xl" role="dialog" aria-label="Quick Note">
				<div class="mx-auto max-w-lg">
					...
					<div class="flex gap-2 overflow-x-auto pb-3 -mx-1 px-1" style="scrollbar-width: none">
						{#each groupedAssignments as [groupName, members] (groupName)}
							...
							<button
								type="button"
								class="shrink-0 rounded-full px-4 py-2 text-sm font-medium transition-colors {quickNoteGroupId === groupId ? `${getGroupColor(groupName)} text-white` : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}"
					...
					<textarea
						...
						class="mt-3 w-full rounded-lg border border-gray-300 px-4 py-3 text-sm focus:border-teal focus:outline-none focus:ring-1 focus:ring-teal"
						rows="2"
					></textarea>
					...
					<div class="mt-3 flex items-center gap-3">
						...
						<button
							type="button"
							class="rounded-lg px-3 py-1.5 text-sm font-medium transition-colors {quickNoteSentiment === 'POSITIVE' ? 'bg-green-500 text-white' : 'bg-green-50 text-green-700 hover:bg-green-100'}"
							onclick={() => quickNoteSentiment = quickNoteSentiment === 'POSITIVE' ? null : 'POSITIVE'}
						>+</button>
						<button
							type="button"
							class="rounded-lg px-3 py-1.5 text-sm font-medium transition-colors {quickNoteSentiment === 'NEUTRAL' ? 'bg-amber-500 text-white' : 'bg-amber-50 text-amber-700 hover:bg-amber-100'}"
							onclick={() => quickNoteSentiment = quickNoteSentiment === 'NEUTRAL' ? null : 'NEUTRAL'}
						>~</button>
						<button
							type="button"
							class="rounded-lg px-3 py-1.5 text-sm font-medium transition-colors {quickNoteSentiment === 'NEGATIVE' ? 'bg-red-500 text-white' : 'bg-red-50 text-red-700 hover:bg-red-100'}"
							onclick={() => quickNoteSentiment = quickNoteSentiment === 'NEGATIVE' ? null : 'NEGATIVE'}
						>!</button>
					</div>
