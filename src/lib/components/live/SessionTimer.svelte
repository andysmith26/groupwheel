...
<div class="rounded-xl bg-white shadow-lg border border-gray-200 overflow-hidden" style="min-width: 200px">
	{#if isExpired}
		<!-- Expired state -->
		<div class="p-4 text-center">
			<p class="text-4xl font-bold text-red-600 animate-pulse">0:00</p>
			...
			<div class="p-4">
				...
				<div class="grid grid-cols-4 gap-2">
					{#each presets as mins}
						<button
							type="button"
							class="rounded-lg bg-gray-100 py-3 text-center text-lg font-semibold text-gray-800 hover:bg-teal hover:text-white transition-colors"
				...
				{#if showCustomInput}
					<div class="mt-3 flex items-center gap-2">
						<input
							...
							min="1"
							max="120"
							class="w-20 rounded-lg border border-gray-300 px-3 py-2 text-sm text-center focus:border-teal focus:outline-none focus:ring-1 focus:ring-teal"
				...
				<div class="mt-3 flex items-center gap-2">
					...
					<button
						type="button"
						class="rounded-full px-2 py-0.5 text-xs font-medium transition-colors {chimeEnabled ? 'bg-teal text-white' : 'bg-gray-200 text-gray-500'}"
