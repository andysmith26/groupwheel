<script lang="ts">
	...
	async function handleImport(pastedText: string) {
		...
		if (!detection.isSimpleNameList) {
			throw new Error(
				'Could not parse the pasted text. Please paste one student name per line.'
			);
		}
	...
	{#if loading}
		<div class="flex flex-1 items-center justify-center">
			<div class="text-center">
				<div class="mx-auto h-8 w-8 animate-spin rounded-full border-2 border-gray-300 border-t-teal-600"></div>
						...
						{:else}
							<div>
								...
								<p class="mt-1 text-sm text-gray-500">
									{students.length} {students.length === 1 ? 'student' : 'students'} in roster.
									Generate groups to get started.
								</p>
							</div>
						{/if}
					</div>
