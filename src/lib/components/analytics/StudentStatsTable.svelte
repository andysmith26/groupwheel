	...
	{:else if students.length === 0}
		<div class="rounded-lg border-2 border-dashed border-gray-300 p-6 text-center">
			...
			<p class="mt-1 text-sm text-gray-400">
				Statistics are calculated from published sessions.
			</p>
		</div>
	{:else}
		<!-- Summary -->
		<div class="flex items-center justify-between">
			<p class="text-sm text-gray-600">
				<span class="font-medium">{students.length}</span> student{students.length === 1 ? '' : 's'} across
		...
		<div class="overflow-x-auto rounded-lg border border-gray-200 bg-white">
			<table class="min-w-full divide-y divide-gray-200">
				<thead class="bg-gray-50">
					<tr>
						<th
							class="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500 cursor-pointer hover:bg-gray-100"
							onclick={() => handleSort('name')}
						>
							Student{getSortIcon('name')}
						</th>
						<th
							class="px-4 py-3 text-center text-xs font-medium uppercase tracking-wider text-gray-500 cursor-pointer hover:bg-gray-100"
					...
					{#each sortedStudents() as student (student.studentId)}
						<tr class="hover:bg-gray-50">
							...
							<td class="whitespace-nowrap px-4 py-3 text-center">
								<span class="rounded-full px-2 py-0.5 text-xs font-medium {getFirstChoiceBadgeClass(student.firstChoicePercent)}">
