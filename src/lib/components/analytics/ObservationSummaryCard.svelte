...
<div class="space-y-6">
  ...
  {:else}
    <!-- Sentiment breakdown -->
    <div>
      <h4 class="mb-3 text-sm font-medium text-gray-700">Sentiment Breakdown</h4>
    ...
    {#if topTags.length > 0}
      <div>
        <h4 class="mb-3 text-sm font-medium text-gray-700">Common Tags</h4>
        <div class="flex flex-wrap gap-2">
          {#each topTags as { tag, count } (tag)}
            <span
              class="inline-flex items-center gap-1 rounded-full bg-teal-50 px-3 py-1 text-sm text-teal-700"
            >
              {tag}
    ...
    {#if recentObservations.length > 0}
      <div>
        <h4 class="mb-3 text-sm font-medium text-gray-700">Recent Observations</h4>
        <div class="space-y-3">
            ...
            <div class="rounded-lg border border-gray-200 bg-white p-3">
              <div class="flex items-start justify-between gap-2">
                <div class="flex min-w-0 items-center gap-2">
                  <span
                    class="flex-shrink-0 rounded px-1.5 py-0.5 text-xs font-medium {getSentimentClass(
                      obs.sentiment
                    )}"
                  >
