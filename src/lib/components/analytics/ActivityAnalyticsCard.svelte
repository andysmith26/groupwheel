...
<button
	type="button"
	class="w-full rounded-lg border border-gray-200 bg-white p-4 text-left shadow-sm transition-shadow hover:shadow-md focus:outline-none focus:ring-2 focus:ring-teal focus:ring-offset-2"
	{onclick}
>
	...
	{#if totalObservations > 0 && sentimentCounts}
		<div class="mt-3 flex items-center gap-3">
			<div class="flex items-center gap-1">
				<span class="inline-flex h-5 w-5 items-center justify-center rounded-full bg-green-100 text-xs font-medium text-green-700">
					+
				</span>
				<span class="text-sm text-gray-600">{sentimentCounts.positive}</span>
			</div>
			<div class="flex items-center gap-1">
				<span class="inline-flex h-5 w-5 items-center justify-center rounded-full bg-gray-100 text-xs font-medium text-gray-700">
					~
				</span>
				<span class="text-sm text-gray-600">{sentimentCounts.neutral}</span>
			</div>
			<div class="flex items-center gap-1">
				<span class="inline-flex h-5 w-5 items-center justify-center rounded-full bg-red-100 text-xs font-medium text-red-700">
					-
				</span>
