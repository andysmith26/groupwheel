...
<div class="rounded-xl border border-gray-200 bg-white">
  <button
    ...
    onclick={() => {
      /* section is always expanded once visible — no collapse toggle */
    }}
  >
    <div>
  ...
  {:else}
    <div class="px-6 pb-6">
      {#if showPrompt}
        <p class="text-sm text-gray-500">Keep recording observations to see trends over time.</p>
      {/if}
        ...
        <div class="space-y-1">
          {#each trends.sessionSummaries as summary (summary.sessionId)}
            ...
            <div>
              <button
                type="button"
                class="flex w-full items-center gap-3 rounded-lg px-2 py-2 text-left transition-colors hover:bg-gray-50 {expandedSessionId ===
                summary.sessionId
                  ? 'bg-gray-50'
                  : ''}"
              ...
              >
                ...
                <!-- Expand indicator -->
                <span
                  class="text-gray-400 transition-transform {expandedSessionId === summary.sessionId
                    ? 'rotate-180'
                    : ''}"
                >
              ...
              {#if expandedSessionId === summary.sessionId}
                <div class="mt-1 mr-4 mb-2 ml-20">
                  {#if loadingSession}
                    <p class="py-2 text-xs text-gray-400">Loading observations...</p>
                  {:else}
                    <ObservationList
