	...
	{:else}
		...
		<div class="flex items-center justify-between">
			<p class="text-sm text-gray-600">
				Based on <span class="font-medium">{totalSessions}</span> published session{totalSessions === 1 ? '' : 's'}
			</p>
		...
		<div class="divide-y divide-gray-200 rounded-lg border border-gray-200 bg-white">
			{#each displayedPairs() as pair (pair.studentAId + ':' + pair.studentBId)}
				<div class="flex items-center justify-between p-3 hover:bg-gray-50">
					...
					<span class="ml-4 flex-shrink-0 rounded-full bg-teal-100 px-2.5 py-0.5 text-xs font-medium text-teal-800">
						{pair.count}x
					</span>
				</div>
			{/each}
