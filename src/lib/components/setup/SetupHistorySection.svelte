    ...
    <div class="space-y-4">
      {#if publishedSessions.length === 0}
        ...
        <div class="rounded-md border border-gray-200 bg-gray-50 p-6 text-center">
          ...
          <p class="text-xs text-gray-400">Groups will appear here after you publish them.</p>
        </div>
      {:else}
        <!-- Session list -->
        <div class="space-y-3">
              ...
              >
                <div class="min-w-0 flex-1">
                  ...
                  <p class="text-sm text-gray-500">
                    {session.academicYear} &middot; Published {formatDate(
                      session.publishedAt ?? session.createdAt
                    )}
                  </p>
                </div>
                <svg
                  class="ml-4 h-5 w-5 text-gray-400 transition-transform {expandedSessionId ===
                  session.id
                    ? 'rotate-180'
                    : ''}"
                  fill="none"
