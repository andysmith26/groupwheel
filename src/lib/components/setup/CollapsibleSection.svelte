...
<section
  class="rounded-lg border bg-white shadow-sm {isPrimary ? 'border-gray-200' : 'border-gray-100'}"
>
  <!-- Header (always visible) -->
  <button
    type="button"
    class="flex w-full items-center justify-between p-4 text-left transition-colors hover:bg-gray-50 focus:ring-2 focus:ring-teal focus:ring-offset-2 focus:outline-none"
  ...
  >
    <div class="flex items-center gap-3">
      ...
      <div>
        <h3 class="text-base font-medium {isPrimary ? 'text-gray-900' : 'text-gray-700'}">
          {title}
          {#if !isPrimary}
