...
>
  {#snippet children()}
    <div class="space-y-4">
      ...
      <!-- Student list -->
      <div
        class="max-h-64 divide-y divide-gray-100 overflow-y-auto rounded-md border border-gray-200"
      >
        ...
        {:else}
          {#each filteredStudents() as student (student.id)}
            ...
            <div
              class="flex items-center justify-between px-3 py-2 hover:bg-gray-50 {removingStudentId ===
              student.id
                ? 'opacity-50'
                : ''}"
            >
      ...
      <p class="text-xs text-gray-500">Edit a student to update their details and preferences.</p>
    </div>
  {/snippet}
</CollapsibleSection>
  ...
  <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
    <div class="mx-4 w-full max-w-sm rounded-lg bg-white p-6 shadow-xl">
      ...
      <p class="mt-2 text-sm text-gray-600">
        Are you sure you want to remove "{getStudentDisplayName(confirmRemoveStudent)}" from this
        roster? They will also be removed from any groups.
      </p>
