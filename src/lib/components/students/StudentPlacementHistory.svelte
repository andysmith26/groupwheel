...
<div class="space-y-6">
  ...
  {:else if placements.length === 0}
    <div class="rounded-lg border-2 border-dashed border-gray-300 p-6 text-center">
      ...
      <p class="mt-1 text-sm text-gray-400">Placements are recorded when sessions are published.</p>
    </div>
  {:else}
    <!-- Summary stats -->
    {#if summary}
          ...
          <div class="flex items-center justify-between p-4">
            <div class="min-w-0 flex-1">
              <div class="flex items-center gap-2">
                ...
                <span
                  class={`rounded-full px-2 py-0.5 text-xs font-medium ${getPreferenceBadgeClass(placement.preferenceRank)}`}
                >
